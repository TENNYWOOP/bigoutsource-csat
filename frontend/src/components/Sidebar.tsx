...
export function Sidebar({ isCollapsed = false, setIsCollapsed }: SidebarProps) {
  ...
  return (
    <aside className={cn("bg-white border-r border-gray-200 h-screen flex flex-col fixed left-0 top-0 transition-all duration-300", isCollapsed ? "w-20" : "w-64")}>
      {/* Logo Area */}
      <div className="h-16 flex items-center px-6 border-b border-transparent mb-4 mt-2 overflow-hidden">
        <div className="flex items-center gap-3">
          ...
          {!isCollapsed && <span className="font-bold text-gray-900 text-lg tracking-tight whitespace-nowrap">Big Outsource CSAT</span>}
        </div>
      </div>
      ...
      <nav className="flex-1 px-4 space-y-1 overflow-y-auto overflow-x-hidden">
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <item.icon className={cn("w-5 h-5 shrink-0", isActive ? "text-gray-300" : "text-gray-400")} />
      ...
      <div className="p-4 border-t border-gray-200">
        {!isCollapsed && (
          <div className="flex items-center gap-3 mb-6 px-2">
            <div className="w-9 h-9 shrink-0 rounded-full bg-gray-100 flex items-center justify-center text-gray-600 font-semibold text-sm border border-gray-200">
              {initials}
            </div>
            <div className="flex flex-col truncate">
              <span className="text-sm font-semibold text-gray-900 leading-none truncate">{user?.name}</span>
              <span className="text-xs text-gray-500 mt-1 uppercase tracking-wider font-medium truncate">
